<!DOCTYPE html>
<html lang="ru">
  <head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" /> 
    <link rel="shortcut icon" href="img/apple-favicon.ico" type="image/x-icon">
    <link rel="icon" href="img/" type="image/x-icon">
    <link rel="stylesheet" href="css/style.css" />
    <title>KUGOO</title>
  </head>
  <body>
    <div class="mobile-menu">
      <ul class="mobile-menu-nav">
        <li class="mobile-menu-nav-item">
          <a href="#" class="mobile-menu-link">Сервис</a>
        </li>
        <li class="mobile-menu-nav-item">
          <a href="#" class="mobile-menu-link">Сотрудничество</a>
        </li>
        <li class="mobile-menu-nav-item">
          <a href="#" class="mobile-menu-link">Заказать звонок</a>
        </li>
      </ul>
      <hr class="mobile-seporator" />
      <div class="mobile-social">
        <a href="#" class="mobile-social-link">
          <img src="img/vk.svg" alt="vk" class="social-link-image" />
        </a>
        <a href="#" class="mobile-social-link">
          <img src="img/telegram.svg" alt="vk" class="social-link-image" />
        </a>
        <a href="#" class="mobile-social-link">
          <img src="img/youtube.svg" alt="vk" class="social-link-image" />
        </a>
      </div>
      <a href="tel:+78005055461" class="mobile-phone">+7 (800) 505-54-61</a>
    </div>
    <nav class="navbar">
      <div class="mobile-menu-toggle">
        <div class="mobile-menu-line"></div>
        <div class="mobile-menu-line"></div>
        <div class="mobile-menu-line"></div>
      </div>
      <div class="container">
        <div class="navbar-nav">
          <ul class="navbar-nav-list">
            <li class="navbar-nav-item">
              <a href="" class="navbar-nav-link">Сервис</a>
            </li>
            <li class="navbar-nav-item">
              <a href="" class="navbar-nav-link">Сотрудничество</a>
            </li>
            <li class="navbar-nav-item">
              <a href="" class="navbar-nav-link">Заказать звонок</a>
            </li>
            <li class="navbar-nav-item navbar-social">
              <a href="#" class="navbar-nav-link">
                <svg class="navbar-social-link" width="12px" height="12x">
                  <use href="img/srite.svg#viber"></use>
                </svg>
              </a>
              <a href="#" class="navbar-nav-link">
                <svg class="navbar-social-link" width="12px" height="12x">
                  <use href="img/srite.svg#whatsap"></use>
                </svg>
              </a>
              <a href="#" class="navbar-nav-link">
                <svg class="navbar-social-link" width="12px" height="12px">
                  <use href="img/srite.svg#telegram"></use>
                </svg>
              </a>
            </li>
          </ul>
          <div class="navbar-phone">
            <a href="tel:+78005055461" class="navbar-phone-link"
              >+7 (800) 505-54-61</a
            >
          </div>
        </div>
        <!-- /.navbar-nav -->
      </div>
      <!-- / container  -->
    </nav>
    <header class="header">
      <div class="container">
        <div class="header-content">
          <p class="header-logo">Kugoo</p>
          <button class="button header-button">
            <svg class="line-svg" width="13px" height="12px">
              <use href="img/srite.svg#line"></use>
            </svg>
            Каталог
          </button>
          <div class="header-search">
            <form class="header-search-form">
              <input
                class="header-search-input"
                type="text"
                placeholder="Искать самокат KUGO"
              />
              <button class="header-search-button" type="reset">
                <svg class="header-search-svg" width="16px" height="16px">
                  <use href="img/srite.svg#search"></use>
                </svg>
              </button>
            </form>
          </div>
          <div class="header-market">
            <a href="#" class="header-market-link">
              <svg class="header-market-icons" width="20px" height="20px">
                <use href="img/srite.svg#love"></use>
              </svg>
            </a>
            <a href="#" class="header-market-link">
              <svg class="header-market-icons" width="20px" height="20px">
                <use href="img/srite.svg#balanc"></use>
              </svg>
            </a>
            <a href="#" class="header-market-link">
              <svg class="header-market-icons" width="20px" height="20px">
                <use href="img/srite.svg#shopping"></use>
              </svg>
            </a>
            <p class="header-market-text">Корзина</p>
          </div>
        </div>
      </div>
      <!-- / container  -->
    </header>
    <!-- / header -->
    <section class="subheader">
      <div class="container">
        <div class="subheader-info">
          <svg class="map-icons" width="15px" height="15px">
            <use href="img/srite.svg#map"></use>
          </svg>
          <address class="subheader-info-address">
            Восточно-Кругликовская улица, 86
          </address>
          <p class="subheader-info-schedule">Вт - Сб 10:00 - 20:00</p>
        </div>
        <h1 class="subheader-title">
          Запишитесь на бесплатный тест-драйв электросамокатов
        </h1>
        <p class="subheader-text">в Москве без ограничения по времени</p>
        <div class="subheader-content">
          <div class="subheader-content-block">
            <a href="#" class="subheader-content-link">
              <svg class="subheader-scooter-icons" width="16px" height="16px">
                <use href="img/srite.svg#scooter"></use>
              </svg>
            </a>
            <p class="subheader-content-text">
              Поймете, какая модель вам подходит
            </p>
          </div>
          <div class="subheader-content-block">
            <a href="#" class="subheader-content-link">
              <svg class="subheader-energy-icons" width="16px" height="16px">
                <use href="img/srite.svg#energy"></use>
              </svg>
            </a>
            <p class="subheader-content-text">
              Проверите лучшие самокаты в деле
            </p>
          </div>
        </div>
        <!-- // subheader-group  -->
        <button class="subheader-button" data-toggle="modal" data-target="#feedback-modal">Записаться</button>
      </div>
    </section>

    <section class="section sample">
      <div class="container">
        <div class="sample-wrap">
          <h2 class="section-title sample-title">
            Определите максимально подходящую вам модель  <br> не теоретически, а на
            практике
          </h2>
          <div class="sample-subtitle">Тест-драйв поможет:</div>
          <ul class="sample-list">
            <li class="sample-item">
              <span class="sample-item-bold">Понять</span> подходит ли вам
              конкретная модель;
            </li>
            <li class="sample-item">
              <span class="sample-item-bold">Испытать</span> самокат в «реальной
              жизни»;
            </li>
            <li class="sample-item">
              <span class="sample-item-bold">Оценить</span> удобство хранения и
              эксплуатации;
            </li>
            <li class="sample-item">
              <span class="sample-item-bold">Узнать</span> реальные
              характеристики и возможности модели.
            </li>
          </ul>
        </div>
        <img
          src="img/sample-bg.jpg"
          alt="sample самокаты"
          class="sample-image"
        />
      </div>
      <!-- // container  -->
    </section>
    <!-- // section sample  -->

    <section class="section sub-sample">
      <div class="subsample-content container">
        <div class="subsample-content-block">
          <h2 class="section-title sub-sample-title">
            Научим правильной <br> и безопасной езде в городе вас или вашего ребенка
          </h2>
          <div class="sample-subtitle">На обучении специалист расскажет::</div>
          <ul class="sample-list">
            <li class="sample-item">Как подготовить самокат к поездке;</li>
            <li class="sample-item">Как «завести» самокат;</li>
            <li class="sample-item">
              Как вести себя во время поездки и обезопасить себя и окружающих;
            </li>
            <li class="sample-item">
              Как складывать и раскладывать электросамокат;
            </li>
            <li class="sample-item">Как ухаживать за своим девайсом.</li>
          </ul>
        </div>
        <img
          src="img/woman-subsample.jpg"
          alt="Женщина на самокате"
          class="subsample-image"
        />
      </div>
      <!-- // container  -->
    </section>
    <!-- //section subsample  -->

    <section class="section trademarks">
      <div class="container">
        <h2 class="section-title trademarks-title">
          Сейчас для тест-драйва и обучения доступны следующие модели
        </h2>
        <!-- Slider main container -->
        <div class="swiper">
          <!-- Additional required wrapper -->
          <div class="swiper-wrapper trademarks-cards">
            <!-- Slides -->
            <div class="swiper-slide trademarks-card">
              <div class="trademarks-card-images">
                <span class="card-benefit">ХИТ</span>
                <a href="#" class="card-balance">
                  <svg class="balance" width="20px" height="20px">
                    <use href="img/srite.svg#love"></use>
                  </svg>
                </a>
                <img
                  src="img/markets.png"
                  alt="самокат"
                  class="card-images-cover"
                />
              </div>
              <div class="trademarks-card-content">
                <div class="card-content-title">Kugoo Kirin M4</div>
                <div class="card-content-features">
                  <ul class="features-list">
                    <li class="features-list-item">
                      <svg class="battery" width="18px" height="18px">
                        <use href="img/srite.svg#battery"></use>
                      </svg>
                      2000 mAh
                    </li>
                    <li class="features-list-item">
                      <svg class="speed" width="18px" height="18px">
                        <use href="img/srite.svg#speed"></use>
                      </svg>
                      60 км/ч
                    </li>
                    <li class="features-list-item">
                      <svg class="power" width="18px" height="18px">
                        <use href="img/srite.svg#power"></use>
                      </svg>
                      1,2 л.с.
                    </li>
                    <li class="features-list-item">
                      <svg class="timer" width="18px" height="18px">
                        <use href="img/srite.svg#timer"></use>
                      </svg>
                      5 часов
                    </li>
                  </ul>
                </div>
                <div class="card-content-footer">
                  <div class="catalog-price">
                    <div class="catalog-old-prices">39 900 ₽</div>
                    <div class="catalog-prices">29 900 ₽</div>
                  </div>
                  <div class="card-footer-link">
                    <a href="#" class="card-link-round">
                      <svg class="markets" width="20px" height="20px">
                        <use href="img/srite.svg#shopping"></use>
                      </svg>
                    </a>
                    <a href="#" class="card-link-round">
                      <svg class="select" width="20px" height="20px">
                        <use href="img/srite.svg#select"></use>
                      </svg>
                    </a>
                  </div>
                </div>
                <button class="trademarks-card-button" data-toggle="modal" data-target="#feedback-modal">
                  Записаться на тест-драйв
                </button>
              </div>
            </div>
            <div class="swiper-slide trademarks-card">
              <div class="trademarks-card-images">
                <span class="card-benefit-green">Новинка</span>
                <a href="#" class="card-balance">
                  <svg class="balance" width="20px" height="20px">
                    <use href="img/srite.svg#love"></use>
                  </svg>
                </a>
                <img
                  src="img/markets.png"
                  alt="самокат"
                  class="card-images-cover"
                />
              </div>
              <div class="trademarks-card-content">
                <div class="card-content-title">Kugoo Kirin M4</div>
                <div class="card-content-features">
                  <ul class="features-list">
                    <li class="features-list-item">
                      <svg class="battery" width="18px" height="18px">
                        <use href="img/srite.svg#battery"></use>
                      </svg>
                      2000 mAh
                    </li>
                    <li class="features-list-item">
                      <svg class="speed" width="18px" height="18px">
                        <use href="img/srite.svg#speed"></use>
                      </svg>
                      60 км/ч
                    </li>
                    <li class="features-list-item">
                      <svg class="power" width="18px" height="18px">
                        <use href="img/srite.svg#power"></use>
                      </svg>
                      1,2 л.с.
                    </li>
                    <li class="features-list-item">
                      <svg class="timer" width="18px" height="18px">
                        <use href="img/srite.svg#timer"></use>
                      </svg>
                      5 часов
                    </li>
                  </ul>
                </div>
                <div class="card-content-footer">
                  <div class="catalog-price">
                    <div class="catalog-old-prices">39 900 ₽</div>
                    <div class="catalog-prices">29 900 ₽</div>
                  </div>
                  <div class="card-footer-link">
                    <a href="#" class="card-link-round">
                      <svg class="markets" width="20px" height="20px">
                        <use href="img/srite.svg#shopping"></use>
                      </svg>
                    </a>
                    <a href="#" class="card-link-round">
                      <svg class="select" width="20px" height="20px">
                        <use href="img/srite.svg#select"></use>
                      </svg>
                    </a>
                  </div>
                </div>
                <button class="trademarks-card-button" data-toggle="modal" data-target="#feedback-modal">
                  Записаться на тест-драйв
                </button>
              </div>
            </div>
            <div class="swiper-slide trademarks-card">
              <div class="trademarks-card-images">
                <span class="card-benefit">ХИТ</span>
                <a href="#" class="card-balance">
                  <svg class="balance" width="20px" height="20px">
                    <use href="img/srite.svg#love"></use>
                  </svg>
                </a>
                <img
                  src="img/markets.png"
                  alt="самокат"
                  class="card-images-cover"
                />
              </div>
              <div class="trademarks-card-content">
                <div class="card-content-title">Kugoo Kirin M4</div>
                <div class="card-content-features">
                  <ul class="features-list">
                    <li class="features-list-item">
                      <svg class="battery" width="18px" height="18px">
                        <use href="img/srite.svg#battery"></use>
                      </svg>
                      2000 mAh
                    </li>
                    <li class="features-list-item">
                      <svg class="speed" width="18px" height="18px">
                        <use href="img/srite.svg#speed"></use>
                      </svg>
                      60 км/ч
                    </li>
                    <li class="features-list-item">
                      <svg class="power" width="18px" height="18px">
                        <use href="img/srite.svg#power"></use>
                      </svg>
                      1,2 л.с.
                    </li>
                    <li class="features-list-item">
                      <svg class="timer" width="18px" height="18px">
                        <use href="img/srite.svg#timer"></use>
                      </svg>
                      5 часов
                    </li>
                  </ul>
                </div>
                <div class="card-content-footer">
                  <div class="catalog-price">
                    <div class="catalog-old-prices">39 900 ₽</div>
                    <div class="catalog-prices">29 900 ₽</div>
                  </div>
                  <div class="card-footer-link">
                    <a href="#" class="card-link-round">
                      <svg class="markets" width="20px" height="20px">
                        <use href="img/srite.svg#shopping"></use>
                      </svg>
                    </a>
                    <a href="#" class="card-link-round">
                      <svg class="select" width="20px" height="20px">
                        <use href="img/srite.svg#select"></use>
                      </svg>
                    </a>
                  </div>
                </div>
                <button class="trademarks-card-button" data-toggle="modal" data-target="#feedback-modal">
                  Записаться на тест-драйв
                </button>
              </div>
            </div>
            <div class="swiper-slide trademarks-card">
              <div class="trademarks-card-images">
                <span class="card-benefit-green">Новинка</span>
                <a href="#" class="card-balance">
                  <svg class="balance" width="20px" height="20px">
                    <use href="img/srite.svg#love"></use>
                  </svg>
                </a>
                <img
                  src="img/markets.png"
                  alt="самокат"
                  class="card-images-cover"
                />
              </div>
              <div class="trademarks-card-content">
                <div class="card-content-title">Kugoo Kirin M4</div>
                <div class="card-content-features">
                  <ul class="features-list">
                    <li class="features-list-item">
                      <svg class="battery" width="18px" height="18px">
                        <use href="img/srite.svg#battery"></use>
                      </svg>
                      2000 mAh
                    </li>
                    <li class="features-list-item">
                      <svg class="speed" width="18px" height="18px">
                        <use href="img/srite.svg#speed"></use>
                      </svg>
                      60 км/ч
                    </li>
                    <li class="features-list-item">
                      <svg class="power" width="18px" height="18px">
                        <use href="img/srite.svg#power"></use>
                      </svg>
                      1,2 л.с.
                    </li>
                    <li class="features-list-item">
                      <svg class="timer" width="18px" height="18px">
                        <use href="img/srite.svg#timer"></use>
                      </svg>
                      5 часов
                    </li>
                  </ul>
                </div>
                <div class="card-content-footer">
                  <div class="catalog-price">
                    <div class="catalog-old-prices">39 900 ₽</div>
                    <div class="catalog-prices">29 900 ₽</div>
                  </div>
                  <div class="card-footer-link">
                    <a href="#" class="card-link-round">
                      <svg class="markets" width="20px" height="20px">
                        <use href="img/srite.svg#shopping"></use>
                      </svg>
                    </a>
                    <a href="#" class="card-link-round">
                      <svg class="select" width="20px" height="20px">
                        <use href="img/srite.svg#select"></use>
                      </svg>
                    </a>
                  </div>
                </div>
                <button class="trademarks-card-button" data-toggle="modal" data-target="#feedback-modal">
                  Записаться на тест-драйв
                </button>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>

    <section class="section cta">
      <div class="bg-grey section-cta">
        <div class="cta-form-wrapper container">
          <form action="handlers.php" method="POST" class="cta-form handlers-form-phone">
            <h2 class="section-title cta-title">
              Нет нужной модели, которую хотите протестировать?
            </h2>
            <p class="cta-form-text">
              Оставьте заявку, и менеджер подберет нужный самокат
            </p>
            <div class="input-group-wrapper">
              <div class="input-group">
                <input
                  id="user-phone"
                  type="tel"
                  class="input phone-mask"
                  name="userphone"
                  placeholder=" +7 (___) __ - __ - __  "
                  required
                />
              </div>
              <div class="input-group">
                <button type="submit"  class="input-group-button">
                  Оставить заявку на тест-драйв
                </button>
              </div>
            </div>
            <div class="cta-checkbox">
              <input
                type="checkbox"
                id="click"
                class="input-checkbox"
                required
                name="checkbox-policy"
              />
              <label for="click" class="cta-checkbox-text">
                Нажимая на кнопку, вы соглашаетесь на обработку персональных
                данных и
                <span class="checkbox-text-span">
                  политикой конфиденциальности</span
                >
              </label>
            </div>
          </form>
          <img src="img/cta-woman.png" alt="coll to action" class="cta-image" />
        </div>
        <!-- // cta-form-wrapper  -->
      </div>
    </section>
    <!--// section cta  -->

    <section class="section finish">
      <div class="container">
        <form action="handlers.php" method="POST" class="finish-form handlers-form-email">
          <h3 class="finish-title">
            Оставьте свою почту и станьте первым, кто получит скидку на новые
            самокаты
          </h3>
          <div class="finish-form-wrapper">
            <div class="finish-group">
              <input
                type="email"
                class="input-email"
                name="useremail"
                placeholder="Введите Ваш email"
                required
              />
            </div>
            <div class="finish-group">
              <button type="submit" class="finish-button">Подписаться</button>
            </div>
          </div>
        </form>
      </div>
    </section>
    <!-- // finish  -->
    <footer class="footer">
      <div class="bg-grey-footer">
        <div class="container">
          <div class="footer-content-top">
            <div class="footer-menu-wrapper footer-fixed">
              <div class="footer-menu-title">Каталог товаров</div>
              <ul class="footer-menu-list">
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Электросамокаты</a>
                </li>
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Электроскутеры</a>
                </li>
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Электровелосипеды</a>
                </li>
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Электровелосипеды</a>
                </li>
              </ul>
            </div>
            <div class="footer-menu-wrapper footer-fixed-2">
              <div class="footer-menu-title">Покупателям</div>
              <ul class="footer-menu-list footer-menu-column-2">
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Сервисный центр</a>
                </li>
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Доставка и оплата</a>
                </li>
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Рассрочка</a>
                </li>
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Тест-драйв</a>
                </li>
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Блог</a>
                </li>
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Сотрудничество</a>
                </li>
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Контакты</a>
                </li>
                <li class="footer-menu-item">
                  <a href="" class="footer-menu-link">Акции</a>
                </li>
              </ul>
            </div>
            <div class="footer-menu-wrapper">
              <div class="footer-menu-title">Контакты</div>
              <div class="footer-service">
                <div class="footer-service-block">
                  <p class="service-text">Call-центр</p>
                  <a href="tel:+78005055461" class="service-number"
                    >+7 (800) 505-54-61</a
                  >
                  <div class="service-schedule">Пн-Вс 10:00 - 20:00</div>
                </div>
                <div class="footer-service-block">
                  <p class="service-text">Сервисный центр</p>
                  <a href="tel:+74993507692" class="service-number"
                    >+7 (499) 350-76-92</a
                  >
                  <div class="service-schedule">Пн-Вс 10:00 - 20:00</div>
                </div>
              </div>
            </div>
            <a href="#" class="button-link">Заказать звонок</a>
          </div>
          <!-- // footer top  -->
          <hr class="seporator-footer" />
          <div class="footer-content-center">
            <div class="footer-social">
              <div class="footer-social-markets">
                <div class="footer-logo header-logo">Kugoo</div>
                <div class="footer-markets-play">
                  <a href="#" class="markets-play-link">
                    <img
                      src="img/google-play.svg"
                      alt="google play"
                      class="markets-play-image"
                  /></a>
                  <a href="#" class="markets-play-link">
                    <img
                      src="img/app-store.svg"
                      alt="app store"
                      class="markets-play-image"
                  /></a>
                </div>
              </div>
              <div class="footer-social-network">
                <a href="#" class="network-link"
                  ><img src="img/vk.svg" alt="vk" class="network-image"
                /></a>
                <a href="#" class="network-link"
                  ><img
                    src="img/youtube.svg"
                    alt="youtube"
                    class="network-image"
                /></a>
                <a href="#" class="network-link"
                  ><img
                    src="img/telegram.svg"
                    alt="telegram"
                    class="network-image"
                /></a>
              </div>
            </div>
          </div>
          <!-- // footer-center  -->
          <hr class="seporator-footer" />
          <div class="footer-bottom">
            <div class="footer-legal">
              <a href="#" class="footer-props">Реквизиты</a>
              <a href="#" class="footer-policy">Политика конфиденциальности</a>
            </div>
            <div class="footer-networks-bottom">
              <div class="footer-networks-cards">
                <svg class="master-card" width="35px" height="24px">
                  <use href="img/srite.svg#g-pay"></use>
                </svg>
                <svg class="master-card" width="35px" height="24px">
                  <use href="img/srite.svg#ap-pay"></use>
                </svg>
                <svg class="master-card" width="35px" height="24px">
                  <use href="img/srite.svg#visa"></use>
                </svg>
                <svg
                  class="master-card master-cards"
                  width="35px"
                  height="24px"
                >
                  <use href="img/srite.svg#master-card"></use>
                </svg>
                <svg class="master-card" width="35px" height="24px">
                  <use href="img/srite.svg#world-card"></use>
                </svg>
                <svg class="master-card" width="35px" height="24px">
                  <use href="img/srite.svg#web-money"></use>
                </svg>
                <svg class="master-card" width="35px" height="24px">
                  <use href="img/srite.svg#qiwi"></use>
                </svg>
              </div>
              <div class="footer-networks-world">
                <p class="world-text">Online чат:</p>
                <div class="world-social">
                  <a href="#" class="world-social-link">
                    <svg class="world" width="16px" height="16px">
                      <use href="img/srite.svg#viber"></use>
                    </svg>
                  </a>
                  <a href="#" class="world-social-link">
                    <svg class="world" width="16px" height="16px">
                      <use href="img/srite.svg#whatsap"></use>
                    </svg>
                  </a>
                  <a href="#" class="world-social-link">
                    <svg class="world" width="16px" height="16px">
                      <use href="img/srite.svg#telegram"></use>
                    </svg>
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
        <!-- / container-footer  -->
      </div>
      <!--// bg grey footer  -->
    </footer>

    <div class="modal" id="feedback-modal">
      <div class="modal-dialog">
        <a
          href="#"
          class="modal-close"
          data-toggle="modal"
          data-target="#feedback-modal"
        >
          <svg class="map-icon" width="20px" height="20px">
            <use href="img/srite.svg#close"></use>
          </svg>
        </a>
        <div class="modal-dialog-content">
          <h3 class="modal-title">Запишитесь натест-драйв электросамоката</h3>
          <p class="modal-subtitle">и подберите модель для себя</p>
          <p class="modal-text">
            Менеджер свяжется c вами в течение 5 минут, чтобы согласовать время.
          </p>
          <form action="handlers.php" method="POST" class="modal-form handlers-form-phone">
            <p class="modal-form-text">Как c вами удобнее связаться?</p>
            <div class="input-group-vertical">
              <div class="modal-input-group">
                <input
                  id="modal-user-phone"
                  type="tel"
                  class="input modal-input phone-mask"
                  placeholder=" +7 (___) __ - __ - __ "
                  name="userphone"
                  required
                />
              </div>
              <div class="modal-input-group">
                <button type="submit" class="modal-form-button">
                  Оформить предзаказ
                </button>
              </div>
            </div>
            <div class="cta-checkbox modal-form-checkbox">
              <input
                type="checkbox"
                id="checkboxs"
                class="input-checkbox"
                required
                name="checkbox-policy"
              />
              <label for="checkboxs" class="cta-checkbox-text modal-checkbox-text">
                Нажимая на кнопку, вы соглашаетесь на обработку персональных
                данных и
                <span class="checkbox-text-modal">
                  политикой конфиденциальности</span
                >
              </label>
            </div>
          </form>
        </div>
        <img
          src="img/modal-dialog.jpg"
          alt="Самокаты"
          class="modal-dialog-image"
        />
      </div>
    </div>

    <div class="modal" id="alert-modal">
      <div class="modal-dialog modal-dialog-thanks">
        <h2 class="modal-title modal-thanks-title">Спасибо за заявку!</h2>
        <a
          href="#"
          class="modal-close"
          data-toggle="modal"
          data-target="#alert-modal"
        >
          <svg class="map-icon" width="20px" height="20px">
            <use href="img/srite.svg#close"></use>
          </svg>
        </a>
        <p class="modal-text thanks-modal-text ">
          Наш менеджер свяжется с Вами в ближайшее время ответит на все
          интересующие вопросы и поможем даже в самых сложных случаях!
        </p>
        <button
          class="button modal-form-button modal-thanks-button"
        >
          Вернуться на главную
        </button>
      </div>
    </div>

    
    <script src="js/swiper-bundle.min.js"></script>
    <script src="js/just-validate.production.min.js"></script>
    <script src="js/main.js"></script>
  </body>
</html>
